// components/ServerSelection.jsx
import { Button, Heading, Input, VStack, HStack, Box } from "@chakra-ui/react";
import { useState } from "react";

export const ServerSelection = ({onServerCreateMenu, onConnect, onBack }) => {
    const [serverName, setServerName] = useState("");
    const [password, setPassword] = useState("");

    const handleConnect = () => {
        onConnect({ serverName, password });
    };
    ...
    return (
        <Box className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
            <VStack spacing="4" width={{ base: "full", sm: "400px" }} bg="white" p="8" borderRadius="md" boxShadow="md">
                <Heading size="lg">Connect to Server</Heading>
                <Input
                    type="text"
                ...
                <HStack spacing="4" width="full">
                    <Button colorScheme="blue" onClick={handleConnect} width="full">
                        Connect
                    </Button>
                ...
                <Button colorScheme="teal" variant="outline" onClick={onServerCreateMenu} width="full">
                        Create server
                </Button>
            </VStack>
        </Box>
